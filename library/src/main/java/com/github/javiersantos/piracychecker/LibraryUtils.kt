...
internal object LibraryUtils {
    ...
    fun buildUnlicensedDialog(context: Context, title: String, content: String): AlertDialog? {
        return (context as? Activity)?.let {
            if (it.isFinishing) return null
            AlertDialog.Builder(context)
                .setCancelable(false)
                .setTitle(title)
                .setMessage(content)
                .setPositiveButton(
                    context.getString(R.string.app_unlicensed_close),
    ...
    fun verifySigningCertificate(context: Context, appSignature: String): Boolean =
        getCurrentSignature(context) == appSignature
    
    fun verifyInstallerId(context: Context, installerID: List<InstallerID>): Boolean {
        val validInstallers = ArrayList<String>()
        val installer = context.packageManager.getInstallerPackageName(
            context.packageName)
    ...
    fun getPirateApp(
        context: Context,
        lpf: Boolean, stores: Boolean, folders: Boolean, apks: Boolean,
        ...
        try {
            ...
            for (app in apps) {
                ...
                if (checkLPF || checkStore || checkOther) {
                    installed = list?.any { it.packageName.contains(app.`package`) } ?: false
                    if (!installed) {
                        installed =
                            isIntentAvailable(context, pm.getLaunchIntentForPackage(app.`package`))
                    }
                }
                if (installed) {
                    theApp = app
     ...
     */
    fun isInEmulator(deepCheck: Boolean): Boolean {
    ...
    private fun getApps(extraApps: ArrayList<PirateApp>): ArrayList<PirateApp> {
        ...
        return ArrayList(apps.distinctBy { it.`package` })
    }
